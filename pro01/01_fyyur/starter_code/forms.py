...
def my_length_check(form, field):
    if len(field.data) > 10 or len(field.data) < 10:
        raise ValidationError('Field must be equial than 10 characters')

class ShowForm(Form):
    artist_id = StringField(
        'artist_id',validators=[DataRequired(), Length(min=4,max=10,message="not right")]
    )
...
class VenueForm(Form):
    ...
    phone = StringField(
        'phone',validators=[DataRequired(),Length(min=4 , max=10, message="must be between 5 to 10")]

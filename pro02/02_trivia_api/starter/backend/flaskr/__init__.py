...
def create_app(test_config=None):
    ...
    def get_categories():
          ...
          formate_categories={}
          # change the formate of categories to be dictionary of dictionaries to be compatibale with frontend
    ...
    def get_question():
        ...
        'total_questions':len(selection), 'categories':formate_categories, 'current_category': None})
    '''
    @TODO:
    ...
    def delete_question(question_id):
        try:
            ...
            # reformate the question so that it will be only 10 questions per page
            current_questions = paginate_books(request, selection)
            return jsonify({'success':True,'deleted_question':question_id,
            'questions': current_questions, 'total_questions':len(selection)})
        except:
            abort(422)
    '''
    @TODO:
    ...
    def create_question():
        ...
        try:
            ...
            # reformate the question so that it will be only 10 questions per page
            current_questions = paginate_books(request, selection)
            return jsonify({'success':True,'created_question':question.id,
            'questions': current_questions, 'total_questions':len(selection)})
        except:
            abort(422)
    '''
    ...
    '''
    # search request that return a list of questions based on the given search term
    ...
    def search_question():
        ...
        try:
            ...
            current_questions = paginate_books(request, selection)
            return jsonify({'success':True,'questions': current_questions,
            'total_questions':len(selection), 'currentCategory':None})
        except:
            abort(422)
    '''
    @TODO:
    ...
    '''
    # a get request that return a list of questions based on a selected category
    ...
    def get_question_by_category(id):
        ...
        if len(current_questions)==0:
            abort(404)
        return jsonify({'success':True,'questions': current_questions,
        'total_questions':len(selection), 'current_category': None})
    '''
    @TODO:
    ...
    '''
    #  an end point that serve a quizz request where return questons to start the quizz. if the quizz has reach the limit of 5 questions
    # or the category has less than than 5 question, it will return a flag that end the quizz
    ...
    def get_quizz():
        ...
        try:
            ...
            if len(questions)==0:
                abort(404)
            # if the quizz reached 5 questions or there is less than 5 question in the selected category and it was played, Quizz to be ended
            if len(previous_questions) == 5 or len(previous_questions) == len(questions):
                random_question= None
            else:
                # check if it is the first round of the quizz or there was already previous rounds played
                if len(previous_questions) > 0:
                    for previous in previous_questions:
                        for question in questions:
                            # a condtion check if the question was already played and remove it from the list of available questions
